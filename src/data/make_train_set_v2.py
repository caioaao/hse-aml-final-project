...
if __name__ == '__main__':
    ...
    shops_sales_train = sales_by_month[['shop_id', 'date_block_num']]\
        .drop_duplicates()

    train_set = pd.DataFrame(columns=['item_id', 'shop_id', 'date_block_num'])

    for month in trange(FIRST_TRAIN_MONTH, 34):
        candidate_shops = pd.DataFrame({'shop_id': test_shops,
                                        'date_block_num': month - 1})
        candidate_shops.merge(shops_sales_train,
                              on=['shop_id', 'date_block_num'])
        shops = candidate_shops['shop_id']
        items = sales_by_month[
            sales_by_month['date_block_num'] == month]['item_id'].unique()
